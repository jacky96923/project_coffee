...
export class StripeService {
    ...
    async createTransaction(user_id: number, shop_id: number, pickupTime: string, total: number) {
        try {
            ...
            let transactionResult = await this.knex("transaction").insert({ user_id: user_id, shop_id: shop_id, payment_status: "in progress", order_time: moment().format("YYYY-MM-DD HH:mm:ss"), pickup_time: moment(pickupDateTime, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss'), pick_status: false, tagged: false, total: total }).returning("id")
