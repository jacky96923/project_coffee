...
export class PromotionInfoService {
  ...
  async getpromotionInfo(shopId: number) {
    try {
      ...
      let images = await this.knex("shop_photos")
        .select(
          "shop_photos.filename as filename",
          "shop_photos.cover_photo as isCover",
          "shop_photos.logo as isLogo"
        )
