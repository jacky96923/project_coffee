...
exports.AllItemService = void 0;
class AllItemService {
    knex;
    constructor(knex) {
        this.knex = knex;
    }
    async getAllItem(shopId) {
        try {
            let items = await this.knex("item")
                .select(this.knex.raw("json_agg(json_build_object('itemId', item.id, 'itemName',item.name, 'itemPhoto', item.item_photo, 'size', item.size, 'price', item.price, 'status', item.is_enabled, 'type', TYPE.name))as item"))
    ...
    async changeItemStatus(checkedItemList, shopId) {
        try {
...
exports.AllItemService = AllItemService;
