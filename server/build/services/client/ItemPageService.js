...
exports.ItemPageService = void 0;
class ItemPageService {
    ...
    async getItemInfo(itemName) {
        ...
        let itemInfo = await this.knex
            .select("item.*", { shopName: "shops.shop_name" }, { address: "shops.address" })
            .from("item")
    ...
    async getInitalState(itemId) {
        let itemState = await this.knex("item")
            .select("shops.id as shopId", "shops.shop_name as shopName", "shops.address as address", "item.id as id", "item.name as name", "item.item_photo as item_photo", "item.size as size", "item.price as price")
            .join("shops", "shops.id", "=", "item.shop_id")
            .where("item.id", itemId);
        let optionState = await this.knex("item")
            .select("option_list.name as optionListName")
            .select(this.knex.raw("json_agg(json_build_object('option_name', custom_option.name, 'price', custom_option.price)ORDER BY custom_option.id) as options"))
            ...
            .join("option_list", "option_list.id", "=", "type_option_list_relation.option_list_id")
            .join("custom_option", "custom_option.id", "=", "option_list.custom_option_id")
            .where("item.id", itemId)
...
exports.ItemPageService = ItemPageService;
