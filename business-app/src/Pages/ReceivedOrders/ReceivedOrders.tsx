...
import getReceivedOrders, { updateCompletedOrder, updatePickupedOrder } from "../../hooks/receivedOrdersAPI"
...
export default function ReceivedOrders () {
    ...
    useEffect(()=>{
        if (Array.isArray(transactions)){
            let chosenTransaction: transaction = transactions.find((entry:any)=>entry.transactionId === orderIdChosen)!
            setOrderChosen(chosenTransaction)
        }
    }, [orderIdChosen])

    const onChoosePendingOrderHandler = (e: React.MouseEvent<HTMLButtonElement, MouseEvent> |  React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.stopPropagation()
        setShowDoneOrder(false)
    ...
    const onChooseDoneOrderHandler = (e: React.MouseEvent<HTMLButtonElement, MouseEvent> |  React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.stopPropagation()
        setShowDoneOrder(true)
        const buttonClicked = e.currentTarget as HTMLButtonElement
        console.log(buttonClicked.id)
        ...
        <div className="flex w-full">
            <div className="relative h-screen w-2/3 mb-20">
                    <div className="h-1/2 overflow-auto border-b-2 border-black">
                        {Array.isArray(pendingOrders)? pendingOrders.map((entry)=>
                                <button key={entry.transactionId} id={String(entry.transactionId)} onClick={(e)=>onChoosePendingOrderHandler(e)} className="m-1.5 w-40 h-14 bg-yellow-500 font-bold text-black">
                                <div id={String(entry.transactionId)} onClick={(e)=>onChoosePendingOrderHandler(e)}>
                                    <div className={`badge ${entry.transactionId!==orderIdChosen?"badge-primary":"badge-secondary"}`}>
                    ...
                    <div className="h-1/2 overflow-auto">
                        {Array.isArray(doneOrders)? doneOrders.map((entry)=>
                                <button key={entry.transactionId} id={String(entry.transactionId)}
                                onClick={(e)=>onChooseDoneOrderHandler(e)}
                                className="m-1.5 w-40 h-14 bg-green-500 font-bold text-black">
                                <div id={String(entry.transactionId)} onClick={(e)=>onChooseDoneOrderHandler(e)}>
                                    <div className={`badge ${entry.transactionId!==orderIdChosen?"badge-primary":"badge-secondary"}`}>
                                        {/* {transactionId?.padStart(4, "0")} */}
                                        {String(entry.transactionId).padStart(4, "0")}
                                    </div>
            ...
            <div className="w-1/3 border-l-2">
                <div className="flex flex-col overflow-auto">
                    <h1 className="text-center my-5 font-bold">現在時間 <span>{clock()}</span></h1>
                    <button className="btn text-2xl mx-auto w-1/2 flex" style={{minWidth: "fit-content"}}>
                        <h1>單號</h1>
                    ...
                    {showDoneOrder? 
                        <div>
                            <h1 className="bg-green-500 text-blue-800 text-center text-xl mt-5 font-bold">請取餐</h1>
                            <h1 className="text-center mt-5 font-bold">
                                取餐時間: <span>{orderChosen?.pickupTime}</span>
                            </h1>
                        </div>:(orderIdChosen === 0? "":
                        <div>
                            <h1 className="bg-yellow-500 text-red-600 text-center text-xl mt-5 font-bold">未完成</h1>
                            <h1 className="text-center mt-5 font-bold">
                    ...
                    <div id="orderList">
                        {orderChosen?.orders.map((entry, idx)=><>
                            <div key={entry.orderId} id={String(entry.orderId)} className="flex justify-center my-5">
                                <h4 className="text-lg fond-bold mr-16">#{idx+1}</h4>
                                <div className="w-2/5">
                                    <h4 className="text-lg fond-bold">{entry.itemName}{entry.itemSize!==null?`(${entry.itemSize})`:""}</h4>
                                    {entry.options.map((option)=><p>{option}</p>)}
                                </div>
                                <h4 className="text-lg fond-bold ml-4">x{entry.quantity}</h4>
                            </div></>
                        )}
                    </div>
                </div>
                <div className="flex justify-center fixed bottom-2 w-1/3">
                    ...
                    {!showDoneOrder? (orderIdChosen === 0? "":<button onClick={onCompleteOrderHandler} className="block mx-auto w-40 h-14 my-1.5 bg-green-500 rounded-2xl font-bold text-black">
                        完成製作
                    </button>):
                    <button onClick={onPickupOrderHandler} className="block mx-auto w-40 h-14 my-1.5 bg-indigo-200 rounded-2xl font-bold text-black">
                        訂單已取
                    </button>}
                </div>
            </div>
